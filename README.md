...
*Note: If your server uses SFTP and requires BOTH a private key and a passphrase (or password), simply provide both `DEPLOY_PASSWORD` and `DEPLOY_PRIVATE_KEY_PATH` in your `.env` file. The CLI will automatically use the password to unlock your key or satisfy the server's two-factor requirement.*
